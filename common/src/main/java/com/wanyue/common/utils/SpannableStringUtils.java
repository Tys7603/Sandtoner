...
{@link Blur#INNER}
        ...
        private void setSpan() {
            ...
            String text=StringUtil.getNotNullString(this.text);
            ...
            if (mTextSize != 0) {
                mBuilder.setSpan(new AbsoluteSizeSpan(mTextSize, true), start, end, flag);
            }


            if (backgroundColor != defaultValue) {
                mBuilder.setSpan(new RoundBackGroundColorSpan(backgroundColor,mTextSize), start, end, flag);
                backgroundColor = defaultValue;
            }
            if (isLeadingMargin) {
            ...
            if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
                if (imageIsBitmap) {
                    mBuilder.setSpan(new ImageSpan(CommonApplication.sInstance, bitmap), start, end, flag);
                    bitmap = null;
                    imageIsBitmap = false;
                } else if (imageIsDrawable) {
                    mBuilder.setSpan(new VerticalImageSpan(drawable), start, end, flag);
                    drawable = null;
                    imageIsDrawable = false;
                } else if (imageIsUri) {
                    mBuilder.setSpan(new ImageSpan(CommonApplication.sInstance, uri), start, end, flag);
                    uri = null;
                    imageIsUri = false;
                } else {
                    mBuilder.setSpan(new ImageSpan(CommonApplication.sInstance, resourceId), start, end, flag);
                    resourceId = 0;
