...
//    }
    ...
    public static String getNotNullString(CharSequence answer) {
        if(answer==null){
            return "";
        }
    ...
    public static String getFormatFloat(String score) {
        if(TextUtils.isEmpty(score)){
            return score;
        }
        try {
            float floatScore=Float.parseFloat(score);
            int integerNum= (int) floatScore;
            /*有小数显示全部，没有小数只显示整数*/
            if(floatScore-integerNum>0){
                return String.valueOf(floatScore) ;
            }else{
