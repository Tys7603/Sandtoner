...
public class CommonHttpUtil {
    ...
    private static <T> Observable<T> requestSingle(HttpMethod httpMethod, String url, HttpParams params, Class<T> cs,final  boolean showMsg) {
        final String finalUrl=appendUrl(url);
        Type type =null;
        if(SeverConfig.jsonType==SeverConfig.FAST_JSON){
        }else{
            type= new ParameterizedTypeImpl(BaseSingleResponse.class, new Class[]{cs});
        }
        Observable<BaseSingleResponse<T>> observable= RxUtils.request(httpMethod, finalUrl, type,params,false);
        return  observable.map(new Function<BaseSingleResponse<T>, T>() {
            ...
            public T apply(BaseSingleResponse<T> baseSingleResponse) throws Exception {
                if(DebugUtil.isDeBug()){
                    String data=JSON.toJSONString(baseSingleResponse.getData());
                    L.e("接口url=="+finalUrl);
                    L.e("接口响应=="+data);
                }
                if(showMsg){
    ...
    public  static  Observable<BaseOriginalResponse> originalRequest(HttpMethod method, String url, HttpParams params){
        final  String finalUrl=appendUrl(url);
        Observable<BaseOriginalResponse> observable=RxUtils.request(method, finalUrl, BaseOriginalResponse.class,params,false)
                .map(new Function<BaseOriginalResponse, BaseOriginalResponse>() {
                    @Override
                    public BaseOriginalResponse apply(@NonNull BaseOriginalResponse baseOriginalResponse) throws Exception {
                        String data=baseOriginalResponse.getData();
                        L.e("接口url=="+finalUrl);
                        L.e("接口响应=="+data);
                        return baseOriginalResponse;
                    }
                })
                ;
        return observable;
