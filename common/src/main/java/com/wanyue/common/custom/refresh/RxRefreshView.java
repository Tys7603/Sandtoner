...
public class RxRefreshView<T> extends FrameLayout implements View.OnClickListener {
    ...
    private void init() {
        ...
        mSmartRefreshLayout.setEnableFooterFollowWhenLoadFinished(false);//是否在全部加载结束之后Footer跟随内容
    ...
    private void initEmptyLayout() {
        if(mEmptyLayout==null){
          if(mEmptyViewProxy!=null){
            ...
            ViewStub stub =null;
            if(mEmptyLevel==0){
                stub = findViewById(R.id.empty_no_data);
          }else {
                stub = findViewById(R.id.empty_no_data2);
            }

            if(stub !=null){
               mEmptyLayout = (ViewGroup) stub.inflate();
    ...
    public void setEmptyViewProxy(BaseProxyMannger mannger, BaseViewProxy viewProxy){
    ...
    public static class ReclyViewSetting{
        ...
        public static ReclyViewSetting createHorizonSetting(Context context,int span){
            ControllLayoutManager linearLayoutManager=new ControllLayoutManager(context, LinearLayoutManager.HORIZONTAL, false);
        ...
        public static ReclyViewSetting createGridSetting(Context context, int spanCount, int divider, final GridLayoutManager.SpanSizeLookup sizeLookup){
            GridLayoutManager gridLayoutManager= new GridLayoutManager(context,spanCount);
