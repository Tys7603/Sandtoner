package com.wanyue.imnew.receiver;
...
import com.wanyue.imnew.util.ImPushUtil;
...
public class JPushReceiver extends BroadcastReceiver {
    ...
    private void onClickNotification(Context context, Intent intent) {
        ...
        if (!CommonAppConfig.isLaunched()) {
            ...
            ActivityManager mAm = (ActivityManager) CommonApplication.sInstance.getSystemService(Context.ACTIVITY_SERVICE);
            //获得当前运行的task
            List<ActivityManager.RunningTaskInfo> taskList = mAm.getRunningTasks(100);
            for (ActivityManager.RunningTaskInfo rti : taskList) {
                //找到当前应用的task，并启动task的栈顶activity，达到程序切换到前台
                if (rti.topActivity.getPackageName().equals(CommonApplication.sInstance.getPackageName())) {
