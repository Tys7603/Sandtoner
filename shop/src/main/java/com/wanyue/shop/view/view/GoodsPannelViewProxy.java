...
/**
 * The type Goods pannel view proxy.
 */
public class GoodsPannelViewProxy extends BaseGoodItemViewProxy implements View.OnClickListener {
    ...
    private CouponGetPopView mCouponGetPopView;
     ...
     * Set data.
     *
     * @param goodsParseBean the goods parse bean
     */
    ...
    private void openCouponPopWindow() {
        if(mStoreGoodsBean==null&&(mCouponGetPopView!=null&&!mCouponGetPopView.isDismiss())){
            return;
        }
        mCouponGetPopView=new CouponGetPopView(getActivity());
        mCouponGetPopView.setGoodsId(mStoreGoodsBean.getId());
