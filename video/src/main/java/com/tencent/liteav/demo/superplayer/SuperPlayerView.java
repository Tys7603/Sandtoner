...
public class SuperPlayerView extends RelativeLayout {
    ...
    public void setShowLocked(boolean showLocked, int limitDurcation) {
        ...
        if(mWindowPlayer!=null){
           mWindowPlayer.setShowLocked(showLocked,limitDurcation);
        }
        if(mFullScreenPlayer!=null){
           mFullScreenPlayer.setShowLocked(showLocked,limitDurcation);
        }
    }
    ...
    private void initView() {
        ...
        mImgCover = (ImageView) mRootView.findViewById(R.id.img_cover);
        ...
        mRootView.removeView(mImgCover);
    ...
    public void setThumb(String thumb){
        if(mImgCover!=null){
           ImgLoader.display(getContext(),thumb,mImgCover);
        }
    }
    ...
    public void hideThumb(boolean isHide){
        if(isHide){
           ViewUtil.setAlpha(mImgCover,0F);
        }else{
            ViewUtil.setAlpha(mImgCover,1.0F);
        }
    }
    ...
    private void fullScreen(boolean isFull) {
        if (getContext() instanceof Activity) {
            ...
            if (isFull) {
                ...
                SystemUtil.setTransparentStatusBar((Activity) getContext());
               /* if (Build.VERSION.SDK_INT > 11 && Build.VERSION.SDK_INT < 19) { // lower api
    ...
    private Player.Callback mControllerCallback = new Player.Callback() {
        ...
        public void onSpeedChange(float speedLevel) {
            ...
            if(mFullScreenPlayer!=null){
               mFullScreenPlayer.setSpeed(speedLevel);
            }
            if(mWindowPlayer!=null){
               mWindowPlayer.setSpeed(speedLevel);
            }
        }
    ...
    private OnPlayerListner mOnPlayerListner;
    public void setOnPlayerListner(OnPlayerListner onPlayerListner) {
        mOnPlayerListner = onPlayerListner;
    }

    public  interface OnPlayerListner{
    ...
    private SuperPlayerObserver mSuperPlayerObserver = new SuperPlayerObserver() {
        @Override
        public void onPlayBegin(String name) {
            if(mOnPlayerListner!=null){
               mOnPlayerListner.onPlayBegin(name);
            }
        ...
        public void onPlayProgress(long current, long duration) {
            ...
            if(mOnPlayerListner!=null){
               mOnPlayerListner.onPlayProgress(current,duration);
            }
    ...
    private void lockVideo(long current, long duration) {
        if(mShowLocked&&current>=mLimitDurcation){
            if(mControllerCallback!=null){
               mControllerCallback.onSeekTo(mLimitDurcation);
