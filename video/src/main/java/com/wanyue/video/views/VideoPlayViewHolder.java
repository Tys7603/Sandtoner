...
public class VideoPlayViewHolder extends AbsViewHolder implements ITXVodPlayListener, View.OnClickListener {
        ...
        switch (e) {
            ...
            case TXLiveConstants.PLAY_EVT_PLAY_END://获取到视频播放完毕的回调
                ...
                if (!mEndPlay) {
                    mEndPlay = true;
                    if (mVideoBean != null) {
                        VideoAPI.videoWatchEnd(mVideoBean.getUid(), mVideoBean.getId());
                    }
                }
                break;
    ...
    public void startPlay(VideoBean videoBean) {
        ...
        VideoAPI.videoWatchStart(videoBean.getUid(), videoBean.getId());
    }
    ...
    public void release() {
        VideoAPI.cancel(VideoHttpConsts.VIDEO_WATCH_START);
        VideoAPI.cancel(VideoHttpConsts.VIDEO_WATCH_END);
        if (mPlayer != null) {
            mPlayer.stopPlay(false);
