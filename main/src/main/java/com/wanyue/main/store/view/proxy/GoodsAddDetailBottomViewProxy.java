...
public class GoodsAddDetailBottomViewProxy extends BaseGoodsDetailBottomViewProxy implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        MainAPI.addConsignmentGoods(id).compose(this.<Boolean>bindToLifecycle()).subscribe(new DialogObserver<Boolean>(getActivity()) {
            @Override
            public void onNextTo(Boolean aBoolean) {
                if(aBoolean){
                   LiveEventBus.get(MainEvent.ADD_GOODS,String.class).post(id);
                   finish();
                }
            }
        });
